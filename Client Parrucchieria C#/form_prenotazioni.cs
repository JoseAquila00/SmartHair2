...
namespace CLIENT_PARRUCCHIERIA
{
    ...
    {
        private string email_utente;//serve per memorizzare l'email dell'utente loggato/registrato
        ...
        private async Task CaricaDisponibilita()
        {
            try
            {
                ...
                foreach (var giorno in checkBoxMap.Keys)
                {
                    ...
                    {//altrimenti ci sono due casi che andiamo a trattare che sono: ci sono tutti gli orari disponibili, ci sono solo alcuni orari disponibili(per quel giorno ovviamente) 
                        //assegno gli orari disponibili ai checkbox
                        for (int i = 0; i < checkBoxList.Count; i++)
                        {
                            if (i < orari.Count)
                            {//controllo utile per capire se sono finiti gli orari disponibili
                ...
                DisabilitaCheckBoxPrecedenti();//abbiamo messo questo metodo qui e non nel costruttore perche caricadisponibilita visto che è async/await attende la risposta del server e di conseguenza il thread principale avanza con il costruttore e quindi mettendolo li non avrebbe funzionato perche caricadisponbilita in quel modo finiva dopo
            }
            catch (Exception ex)
            {
        ...
        private void DisabilitaCheckBoxPrecedenti()
        {
            ...
            for (int i = 0; i < indiceGiornoAttuale; i++)//se è martedi non entra proprio nel for
            {
                ...
                string giorno = giorniSettimana[i]; //per i=1 assume martedi ecc
        ...
        private void AssociaEventiCheckbox()
        {
            ...
            {//this.Controls è una collezione che contiene tutti i controlli presenti nel form(TUTTI quindi bottoni,checkbox,label ecc)
                //siccome i checkbox sono dentro panel allora devo entrare all'interno di ogni panelpanel e poi cercare i checkbox
                if (control is Panel)
                {// questo è un controllo di matching, se è checkbox allora associo il metodo Checkbox_Changed all'evento checkedChanged di ogni checkbox ogni volta che ne trovo uno
                    foreach (Control subcontroll in control.Controls)
                    {//vado a scorrere tutti controlli contenuti in panel
        ...
        private async Task InviaPrenotazioneAlServer(string email, string giorno_prenotazione, string orario, string servizio, CheckBox checkBox)
        {
            ...
            {//se c'è errore di connessione allora
                MessageBox.Show("Errore: " + ex.Message, "Errore", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Application.Exit();
            }
        ...
        private string MostraSceltaServizio()
        {
            ...
            };//sto creando una list box che va a coprire tutto il form grazie a questo fill
        ...
        //metodo di gestione dell'evento mouseclick solo sulla colonna elimina
