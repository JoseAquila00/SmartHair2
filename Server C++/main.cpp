...
static sql::mysql::MySQL_Connection* connectToDatabase() {
    try {
        sql::mysql::MySQL_Driver* driver = sql::mysql::get_mysql_driver_instance(); // Ottiene l'istanza del driver MySQL, qui non serve fare il delete perche questa funzione implementa il pattern singleton cioe get_mysql_driver_istance() infatti esso non si crea un nuovo oggetto ogni volta, ma restituisce sempre la stessa istanza se � presente. in qualche parte del codice ci sara uno static che assicura che esista una sola istanza per tutta la durata del programma 
...
static std::string simpleHash(const std::string& password) {
    unsigned int hash = 0;//int pu� contenere numeri negativi, quindi se il valore diventa troppo grande, potrebbe diventare negativo (overflow aritmetico).unsigned int non ha valori negativi e permette di lavorare con numeri pi� grandi senza il rischio di segni negativi.
    for (char c : password) {
        hash = (hash * 31) + static_cast<unsigned int>(c);//moltiplico per 31 perche � un numero primo per evitare collisioni. aggiungiamo poi il valore ASCII del carattere tramite lo static cast(per esempio il carattere c � 99) 
...
static crow::json::wvalue* handleDisponibilita(sql::mysql::MySQL_Connection* conn) {
    ...
    try {
        ...
        //se non ci sono disponibilita, restituiamo un messaggio dicendo che � tutto occupato
        if (disponibilita.empty()) {
            (*response)["success"] = true;
            (*response)["message"] = "non ci sono posti disponibili";
            (*response)["disponibilita"] = crow::json::wvalue::list();
            return response;//qui ho messo una lista vuota che guardando l'implementazione di wvalue � un vettore di oggetti wvalue
        }

        //se invece ci sono disponibilita allora andiamo avanti e facciamo la seguente. questo response, come entry, con l'override delle [], � un oggetto di tipo wvalue 
        (*response)["success"] = true;
...
static crow::json::wvalue* handleRegistration(sql::mysql::MySQL_Connection* conn, const std::string& nome, const std::string& cognome, const std::string& email, const std::string& password) {
    ...
    sql::ResultSet* res = nullptr;//ho messo res come resultset perche restituisce un puntatore a resultset quella funzione che poi andro ad eseguire, che pero � una classe astratta. quindi il metodo next e getInt verranno implementati nella classi derivate di resultset che � mysql_Resultset. Una classe � astratta se dichiara almeno un metodo virtuale puro (cio� virtual ... = 0;). Nel tuo header quasi tutti i metodi sono dichiarati come = 0, quindi ResultSet non pu� essere istanziata direttamente.
    ...
    try {
        ...
        if (res->next()) { //MySQL_ResultSet eredita sql::ResultSet, quindi il puntatore res pu� contenere un oggetto MySQL_ResultSet, quindi saranno implementati in mysql_resultset.cpp.
...
static crow::json::wvalue* handleAggiungiPrenotazione(sql::mysql::MySQL_Connection* conn, const std::string& email, const std::string& giorno_prenotazione, const std::string& ora_prenotazione, const std::string& servizio, const double& prezzo) {
    ...
    try {
        ...
        //CONTROLLO SE L'ORARIO � DISPONIBILE BLOCCANDOLO E QUINDI ACQUISENDO IL LOCK
        stmt = conn->prepareStatement("SELECT * FROM disponibilita WHERE giorno_disponibile = ? AND ora_disponibile = ? FOR UPDATE");//Il comando FOR UPDATE in SQL serve per bloccare la riga selezionata finch� la transazione non � completata. Questo � utile per evitare problemi di concorrenza quando pi� utenti cercano di prenotare lo stesso orario contemporaneamente.
        ...
        if (!res->next())
        {
            ...
            //annullo la transazione se l'orario non � disponibile
...
static crow::json::wvalue* handleEliminaTutteLePrenotazioniEDisponibilita(sql::mysql::MySQL_Connection* conn) {
    ...
    try {
        ...
        (*response)["success"] = true;
        (*response)["message"] = "Prenotazioni e disponibilit� eliminate con successo.";
        return response;
    }
    catch (sql::SQLException& e) {
        transStmt = conn->createStatement();
...
static crow::json::wvalue* handleAggiungiDisponibilita(sql::mysql::MySQL_Connection* conn, const std::string& giorno, const std::string& ora) {
    ...
    try {
        ...
        delete stmt;
        (*response)["success"] = true;
        (*response)["message"] = "disponibilit� aggiunta con successo";
        return response;
    }
    catch (sql::SQLException& e) {
        delete stmt;
...
static crow::json::wvalue* handleAggiungiDisponibilitaAdmin(sql::mysql::MySQL_Connection* conn, const std::string& giorno) {
    ...
    try {
        ...
        (*response)["success"] = true;
        (*response)["message"] = "Disponibilit� aggiunte correttamente per il giorno scelto.";
        return response;
    }
    catch (sql::SQLException& e) {
        delete stmt;
...
int main() {
    ...
    CROW_ROUTE(app, "/register").methods(crow::HTTPMethod::POST)([raw_conn](const crow::request& req) {
        ...
        if (!body) { // Controlla se il corpo JSON � valido (crow/json.h)
