...
class LoginWindow:
    ...
    def check_login(self):
        email=self.email_entry.get() #ottengo l'email inserita rimuovendo spazi extra
        ...
        if not re.match(email_regex, email):
            ...
            return  # Blocca l'esecuzione se l'email non   valida
        ...
        if response.get("success"): #se success   true
            messagebox.showinfo("SUCCESSO", response["message"]) #mostro il messaggio del server che   andato a buon fine il login
...
if __name__=="__main__":
    root=tk.Tk() #questo   il contenitore principale della GUI e quindi root   l'oggetto che rappresenta la finestra principale, tutte le cose(bottoni ecc) vengono posizionati dentro root. se io eseguo solo il codice import di tkinter, questa riga e root.mainloop() si apre una finestra vuota
    app=LoginWindow(root) #  una classe che gestisce l'interfaccia di login e usera root per costruire l'interfaccia dentro la finestra principale
