...
class Dashboard:
    def __init__(self,root):
        ...
        # Crea un bottone per aggiungere disponibilit 
        ...
        self.root.protocol("WM_DELETE_WINDOW", self.conferma_uscita) #WM_DELETE_WINDOW   un protocollo di Tkinter che viene attivato quando l'utente tenta di chiudere la finestra con la X. Viene sostituito il comportamento predefinito della chiusura con la funzione self.conferma_uscita 
    ...
    def filtra_clienti(self):
        ...
        email_ricercata=self.search_entry.get().strip().lower() #qui ho aggiunto pure Strip rispetto a prima, perch  cos  se metto spazi all'inizio e alla fine della stringa me li leva
        ...
        clienti = response.get("clienti")#clienti   una lista di dizionari

        #se il campo   vuoto, mostra tutti i clienti e interrompe l'esecuzione
        ...
        cliente_trovato = next((c for c in clienti if email_ricercata==c["email"].lower()),None)#   un generatore
        #cliente trovato sar  un singolo dizionario (e anche unico) della lista di dizionari, oppure sar  None
        ...
        self.riempi_tabella([cliente_trovato]) #gli sto passando una lista che sto creando qui con un solo elemento che   cliente_trovato che avra nome, cognome e email
    ...
    def aggiungi_giorni_specifici(self):
        ...
        #adesso creo un dizionario comprehension giorni  e ogni giorno della settimana viene usato come chiave e a ciascuna chiave associo un'istanza della classe intvar() che   una variabile speciale di tkinter  utilizzata solitamente con widget interattivi. questo intvar()   una classe quindi creo un'istanza della classe e questo serve perche ad ogni giorno associo un checkbox e se il checkbox   selezionato intvar dovrebbe assumere il valore 1 altrimenti 0 e cosi raccolgo tutti i checkbox selezionati 
        giorni={giorno: tk.IntVar() for giorno in ["Martedi","Mercoledi","Giovedi","Venerdi","Sabato"]}
        

        #ora vado a iterare il dizionario e associo ad ogni elemento un checkbutton che ha come testo il giorno e quindi la chiave e come variabile l'istanza e questo ci permettera di verificare se la checkbox   stata selezionata o meno
    ...
    def mostra_prenotazioni(self):
        ...
        response=get_prenotazioni()
        #se la risposta   none , ci sono problemi con il server
        if response is None:
            return

        #se il campo vuoto   true, significa che non ci sono prenotazioni disponibili
        if response.get("vuoto"):
            messagebox.showwarning("Attenzione",response.get("message"))
            return
    ...
    def riempi_tabella_prenotazioni(self,prenotazioni):#prenotazion è una lista di dizionari
    ...
    def filtra_prenotazioni(self):
        ...
        # controllo se il servizio   valido
